...
Пусть $\rnk(\mathbb{X}) = d$, $(z_1, \ldots, z_d)$ --- ортогональный базис в в подпространстве размерности $d$ --- $\spn(X_1, \ldots, X_p) = \mathrm{colspan}(\mathbb{X})$.
Превратим базис в \textit{ортонормированный базис}: $Q_i = \frac{z_i}{\Arrowvert z_i\Arrowvert}$.\footnote{Рассмотрим пример: пространство $\mathbb{R}^2$, соответствующий базис $(1,1)^{\mathrm{T}}/\sqrt{2}$, $(1,-1)^{\mathrm{T}}/\sqrt{2}$. Это ортонормированный базис. Как вычислить координаты вектора $(5, 4)^{\mathrm{T}}$ в данном базисе?\\
...
\end{gather}
$\Arrowvert Q_j\Arrowvert = 1$, $\Arrowvert F_j\Arrowvert \neq 1$ в разложении \ref{2}. Давайте нормируем $F$. Пусть $\sigma_j = \Arrowvert F_j\Arrowvert$, $P_j = \frac{F_j}{\Arrowvert F_j\Arrowvert}$, тогда $\Arrowvert Q_j\Arrowvert = \Arrowvert P_j\Arrowvert = 1$ и
...
\end{gather}
\textit{Что за матрица $Q_jP_j^{\mathrm{T}} \in \mathbb{R}^{n \times p}$?} Можно заметить, что $\rnk(Q_jP_j^{\mathrm{T}}) = 1$.\footnote{Столбцы пропорциональны.} Таким образом, у нас была исходная матрица ранга $d$, а мы превратили её в сумму $d$ элементарных матриц ранга 1.
...
Умножение на матрицу $\mathbb{U}$ означает \textit{поворот} или \textit{отражение}. Пусть есть вектора $Y$, $Z$, после умножения на матрицу $\mathbb{U}$ получим $\widetilde{Y} = \mathbb{U}Y,~ \widetilde{Z}=\mathbb{U}Z$.\footnote{Поворачиваем вектора $Y$ и $Z$ на какой-то угол, при умножении на матрицу $\mathbb{U}$ не меняются нормы векторов.}
\begin{example}
	...
	\end{pmatrix}$.
\end{example}
\item Пусть $\{P_i\}_{i = 1}^r$ --- система независимых векторов, рассмотрим линейную оболочку $\mathcal{L}_r = \spn\{P_1, \ldots, P_r\}$ в $\mathbb{R}^L$, $\Pi: \mathbb{R}^L \rightarrow \mathcal{L}_r$ --- проектор на $\mathcal{L}_r$. Матрица $\Pi$: 
\begin{gather*}
...
Перейдём к $\mathbb{X} \in \mathbb{R}^{n \times p}$ ($L \rightarrow p$ --- количество индивидов, $K \rightarrow n$ --- число признаков). Пусть $\mathbb{X}$ --- центрированы. \textit{Берём признак, какую норму нужно рассматривать?} Вероятностную норму. \textit{Что означает характеристика $\Arrowvert X_i\Arrowvert_2^2$ на статистическом языке? }
\begin{gather*}
...
\begin{gather*}
\Arrowvert\mathbb{X}\Arrowvert_{1,2}^2 = \sum\limits_{i = 1}^p \Arrowvert X_i\Arrowvert_2^2 = \sum\limits_{i = 1}^p s^2(X_i) \text{ --- total variance.}
\end{gather*}
...
\begin{equation*}
\cos^2(\alpha(Y_i,\spn(U_1,U_2)))=\frac{\overbrace{\langle Y_i,U_1\rangle^2}^{z_{i1}^2}}{\Arrowvert Y_i \Arrowvert^2} + \frac{\overbrace{\langle Y_i,U_2\rangle^2}^{z_{i2}^2}}{\Arrowvert Y_i \Arrowvert^2} = \frac{z_{i1}^2}{\sum\limits_{j=1}^d z_{ij}^2} + \frac{z_{i2}^2}{\sum\limits_{j=1}^d z_{ij}^2}.
\end{equation*}
...
\begin{proposition}
	$\mathbb{Y}=\mathbb{X}^\mathrm{T}$.
	 ...
	 Задача $\Arrowvert \mathbb{Y}-\widetilde{\mathbb{Y}}\Arrowvert \to \min \limits_{\widetilde{\mathbb{Y}}:~\rnk\widetilde{\mathbb{Y}}\le r}$\footnote{решение этой задачи --- сумма собственных троек, соответствующих первым $r$ главным компонентам.} эквивалентна задаче $\Arrowvert \mathbb{Y}\mathbb{Y}^\mathrm{T}-\widetilde{\mathbb{Y}}\widetilde{\mathbb{Y}}^\mathrm{T}\Arrowvert \to \min \limits_{\widetilde{\mathbb{Y}}:~\rnk\widetilde{\mathbb{Y}}\le r}$.
\end{proposition}
...
Пусть $\xi=(\xi_1,\ldots,\xi_p)^\mathrm{T} \in \mathbb{R}^p, ~\eta=(\eta_1,\ldots,\eta_r)^\mathrm{T}\in\mathbb{R}^r$, $r<p$. Предполагаем, что $\mathbb{E}\xi_i=0,$ $\mathbb{E}\eta_i=0, ~\mathbb{D}\eta_i=1,$ $\eta_i$ и $\eta_j$ некоррелированы. Пусть $\mathbb{F}_r=[F_1:\ldots:F_r]\in\mathbb{R}^{p\times r}$.

Итак, модель в факторном анализе: $\xi=\mathbb{F}_r\eta+\mathcal{E}, ~ \text{Cov}\mathcal{E}=\diag(\sigma_1^2,\ldots,\sigma_p^2)$, $\eta$ и $\mathcal{E}$ некоррелированы. Случайной величине $\xi$ соответствует матрица данных $\mathbb{X}^\mathrm{T}$ (признаки, которые мы наблюдаем), а $\eta$ соответствует $\mathbb{V}_r^\mathrm{T}$ (скрытые признаки, которые мы не наблюдаем). Перед нами стоит задача выявить эти скрытые признаки (задача факторного анализа).

Модель можно переписать в следующем виде:
\begin{equation*} \text{Cov}(\xi)=\text{Cov}(\mathbb{F}_r\eta)+\text{Cov}(\mathcal{E})=\mathbb{F}_r\mathbb{F}_r^\mathbb{T}+\Psi,
\end{equation*}
где $\Psi=\diag(\sigma_1^2,\ldots,\sigma_p^2), ~\rnk \mathbb{F}_r\mathbb{F}_r^\mathbb{T}\le r. $ Тогда данные --- это выборочная ковариационная матрица. Пусть $f_{ij}=\rho(\xi_i,\eta_j)$, тогда матрица выглядит следующим образом:
...
Мы предполагаем, что в каждой строчке перед $\varepsilon_i$ только один из коэффициентов не равен нулю, а среди $f_{ij}$ хотя бы два не равны нулю.

\section{Общности и уникальности}

В этом пункте введем понятие общности и уникальности признаков.

Обозначим $\xi'= \mathbb{F}_r \eta,$ тогда $\xi = \xi'+\varepsilon.$ Предполагаем, что $\mathrm{D}\xi_i=1,$ $\mathrm{D}\varepsilon_i=\sigma_i^2,$ тогда $\mathrm{D}\xi_i'=\mathrm{D}\xi_i-\mathrm{D}\varepsilon_i= 1-\sigma_i^2.$ Тогда
\begin{equation*}
\mathbb{F}_r\mathbb{F}_r^\mathrm{T} = \text{Cov} (\xi') = 
\left(\begin{matrix} 
...
Можно заметить, что $\text{Cov}(\xi_i'+\varepsilon_i, \xi_j'+\varepsilon_j) = \text{Cov}(\xi_i', \xi_j').$\footnote{Остальные слагаемые обнулятся.} Также известно, что ковариационная матрица $\xi'$:  $\text{Cov} (\xi') = \mathbb{F}_r \text{Cov}(\eta) \mathbb{F}_r^\mathrm{T} = \mathbb{F}_r\mathbb{F}_r^\mathrm{T}.$
...
В матрице $\mathbb{F}$ не может быть такого столбца, где только один элемент не равен нулю, то есть это факторы, соответствующие хотя бы двум признакам. $\mathcal{E}$ в каком-то смысле тоже фактор, но он соответствует только одному признаку. Факторный анализ ищет общие факторы.
...
\begin{equation*}
\frac{(p-r)^2-(p+r)}{2}\le 0.
\end{equation*}

\section{Задача оценивания параметров}
Теперь можем перейти к задаче оценивания параметров.
...
\textit{Алгоритм MINRES} (OLS для ковариационной матрицы):
\begin{enumerate}
...
\end{equation*}
 где минимизируется сумма по всем элементам, кроме диагональных, $\widetilde{\mathbb{S}}$ --- симметричная, неотрицательно определенная матрица ранга $r$
...
\end{equation*}
минимизируется сумма по всем элементам, $\widetilde{\mathbb{S}}$ --- симметричная, неотрицательно определенная матрица ранга $r$.

Таким образом, цель факторного анализа --- как можно лучше воспроизвести ковариации. В анализе главных компонент стоит аппроксимизационная задача, и АГК не решает задачу факторного анализа.
...
Идеальное значение равно $0$, критическая область справа.\footnote{Логарифм будет равен нулю, если данные идеально соответствуют гипотезе.}
...
Можно применять информационные критерии AIC, BIC (в нормальной модели) --- подход со штрафом за число параметров. Можно промоделировать, посчитать вклады и строить доверительный интервал.
...
Заметим, что если $\mathbb{F}_r$ --- решение, то $\widetilde{\mathbb{F}}_r=\mathbb{F}_r\mathbb{W}$ --- тоже решение.\footnote{Только будут другие корреляции.} $\mathbb{W}$ ортогональна.

На выборочном языке: $\mathbb{X}'$ --- та часть исходных данных, которая описывается факторами ($\xi'=\mathbb{F}_r\eta$).
\begin{equation*}
\mathbb{X}'=\mathbb{V}\mathbb{F}^\mathrm{T}=\underbrace{\mathbb{V}\mathbb{W}}_{\widetilde{\mathbb{V}}}\underbrace{\mathbb{W}^\mathrm{T}\mathbb{F}^\mathrm{T}}_{\widetilde{\mathbb{F}}},
\end{equation*}
$\widetilde{\mathbb{V}}$ --- новые факторы, $\widetilde{\mathbb{F}}$ --- новые факторные нагрузки. Хотелось бы найти такую $\mathbb{W},$ чтобы интерпретация была как можно проще.

\textit{Метод вращений Varimax}:
...
\end{equation*}
где мы рассматриваем $f_{ij}^2$, т.к. $f_{ij}$ могут быть отрицательны, и считаем выборочную дисперсию.

\textbf{На выборочном языке:} $\mathbb{V}=[V_1:\ldots:V_r], ~ V_1,\ldots,V_r\in\mathbb{R}^n$ --- факторные вектора. На выборочном языке $\xi$ переходит в $\mathbb{X}\in\mathbb{R}^{n\times p}$, $\xi'$ переходит в $\mathbb{X}'\in\mathbb{R}^{n\times p}$, $\eta$ (соответствует факторам) переходит в $\mathbb{V}\in\mathbb{R}^{n\times r}$. Тогда $\mathbb{F}\eta$ соответствует $\mathbb{V}\mathbb{F}^\mathrm{T}$.

Поворачиваем факторы: $\widetilde{\eta}=\mathbb{W}\eta$ --- факторы после вращения, $\widetilde{\mathbb{F}}=\mathbb{F}\mathbb{W}^{-1},$ где $\mathbb{W}\in\mathbb{R}^{r\times r}$ --- матрица поворота. Что означает на языке векторов то, что мы поворачиваем факторы? Это значит, что $\widetilde{\mathbb{V}}=\mathbb{V}\mathbb{W}\in\mathbb{R}^{n\times r}$.

Идея: мы хотим, чтобы $\mathbb{F}\eta=\widetilde{\mathbb{F}}\widetilde{\eta}$ (то есть $\xi'$ не должно меняться). И действительно, 
\begin{equation*}
\widetilde{\mathbb{F}}\widetilde{\eta}=\mathbb{F}\mathbb{W}^{-1}\mathbb{W}\eta=\mathbb{F}\eta,
\end{equation*}

мы поменяли факторные веса и получили то же самое.\footnote{В одну сторону поворачиваем факторы и в другую сторону поворачиваем $\mathbb{F}$.}

В случае ортогональных вращений: $\mathbb{W}^{-1}=\mathbb{W}^{\mathrm{T}}$ и тоже матрица поворота.


\section{Косоугольные вращения}

Вообще говоря, $\mathbb{W}^{-1}\ne\mathbb{W}^{\mathrm{T}}$. Мы хотим, чтобы новые факторы после вращения были по-прежнему нормированы (то есть дисперсия была равна 1 на языке случайных величин). Мы предполагаем, что все центрировано. Тогда
\begin{equation*}
\text{Cov}(\widetilde{\eta})=\mathrm{E}\widetilde{\eta}\widetilde{\eta}^\mathrm{T} = \mathrm{E}(\mathbb{W}\eta\eta^\mathrm{T}\mathbb{W}^\mathrm{T}) = \mathbb{W} \underbrace{\text{Cov}\eta}_{\mathbb{I}_{r\times r}} \mathbb{W}^\mathrm{T} = \mathbb{W}\mathbb{W}^\mathrm{T},
\end{equation*}
будем требовать, чтобы на диагонали этой матрицы стояли единицы.

\begin{remark}
	Необходимо проверять, чтобы в корреляционной матрице новых факторов были не очень большие по модулю корреляции, т.к. чем больше корреляции между новыми признаками, тем более подозрительный результат.
\end{remark}
...
\begin{definition}
	Факторный паттерн (factor pattern, factor loadings) --- это матрица $\widetilde{\mathbb{F}}$, коэффициенты линейной комбинации в $\xi'=\widetilde{\mathbb{F}}\widetilde{\eta}.$
\end{definition}
...
\begin{equation*}
\Phi = \text{Cov}(\xi,\widetilde{\eta})=\mathrm{E}(\xi'\widetilde{\eta}^\mathrm{T}) = \mathrm{E}(\widetilde{\mathbb{F}}\widetilde{\eta}\widetilde{\eta}^\mathrm{T}) = \widetilde{\mathbb{F}}\text{Cov}(\widetilde{\eta}) =  \widetilde{\mathbb{F}}\mathbb{W}\mathbb{W}^\mathrm{T}.
\end{equation*} 
...
\begin{enumerate}
	...
	\vdots \\
	v_{ir}
	\end{matrix} \right) = (\mathbb{F}^\mathrm{T}\mathbb{F})^{-1}\mathbb{F}^\mathrm{T}Y_i
	\end{equation*}
	...
	\vdots \\
	v_{ir}
	\end{matrix} \right) = (\mathbb{F}^\mathrm{T}\Psi^{-1}\mathbb{F})^{-1}\mathbb{F}^\mathrm{T}\Psi^{-1}Y_i
	\end{equation*}
	...
	\vdots \\
	v_{ir}
	\end{matrix} \right) = \mathbb{F}^\mathrm{T}\mathbb{S}^{-1}Y_i
	\end{equation*}
