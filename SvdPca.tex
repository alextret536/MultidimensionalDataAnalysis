...
Обозначим $\xi'= \mathbb{F}_r \eta,$ тогда $\xi = \xi'+\varepsilon.$ То есть $\xi'$ --- это та часть данных, которая описывается факторами. Предполагаем, что $\mathrm{D}\xi_i=1,$ $\mathrm{D}\varepsilon_i=\sigma_i^2,$ тогда $\mathrm{D}\xi_i'=\mathrm{D}\xi_i-\mathrm{D}\varepsilon_i= 1-\sigma_i^2.$ 

Можно заметить, что $\text{Cov}(\xi_i, \xi_j)=\text{Cov}(\xi_i'+\varepsilon_i, \xi_j'+\varepsilon_j) = \text{Cov}(\xi_i', \xi_j').$\footnote{Остальные слагаемые обнуляются из-за независимости.} Получаем, что $\text{Cov}(\xi_i', \xi_j')=\rho_{ij}$. Также известно, что ковариационная матрица $\xi'$ равна $\text{Cov} (\xi') = \mathbb{F}_r \text{Cov}(\eta) \mathbb{F}_r^\mathrm{T} = \mathbb{F}_r\mathbb{F}_r^\mathrm{T}.$
...
Заметим, что если $\mathbb{F}_r$ --- решение, то вращение $\widetilde{\mathbb{F}}_r=\mathbb{F}_r\mathbb{W}$ --- тоже решение.\footnote{Только будут другие корреляции.} Матрица $\mathbb{W}$ ортогональна.
...
\begin{example}
	...
	\begin{minipage}{0.51\linewidth}
		\centering
		\includegraphics[width=200pt, height=180pt]{p12}
...
\end{equation*}
где мы рассматриваем $f_{ij}^2$, т.к. $f_{ij}$ могут быть отрицательны, и считаем выборочную дисперсию для $f_{ij}^2$.\footnote{Мы хотим, чтобы в матрице были большие и маленькие числа, а количественная характеристика, которая позволяет отличать одинаковые числа от разных --- это дисперсия, ее мы и максимизируем.}
...
\begin{definition}
	Факторный паттерн (factor pattern, factor loadings) --- это матрица $\widetilde{\mathbb{F}}$, коэффициенты линейной комбинации в $\xi'=\widetilde{\mathbb{F}}\widetilde{\eta}$ (коэффициенты линейной комбинации, с которыми исходные признаки выражаются через новые).
\end{definition}
